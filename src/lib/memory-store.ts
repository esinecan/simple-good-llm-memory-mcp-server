...
import { getEmbeddingService, GoogleEmbeddingService } from './embeddings.js';
  ...
  async storeMemory(text: string, metadata: MemoryMetadata, id?: string): Promise<string> {
    ...
    try {
      ...
      const memoryId = id || `mem_${Date.now()}_${Math.random().toString(36).substring(2, 10)}`;
        // Prepare metadata for storage - preserve all incoming metadata
      const fullMetadata = {
        ...metadata, // Spread all incoming metadata fields first
        text, // Add/overwrite text from parameter
        timestamp: metadata.timestamp || Date.now(), // Ensure timestamp exists
        textLength: text.length // Update textLength based on actual text
      };
      ...
      if (results.ids && results.ids.length > 0) {
        for (let i = 0; i < results.ids.length; i++) {
          ...
          const document = results.documents[i];
            ...
            metadata: metadata as any // Return full metadata to preserve all fields
          });
        }
      }

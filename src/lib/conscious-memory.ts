...
import { generateEntityId } from './rule-based-extractor.js'; // Assuming this is exported and useful
import { withRetry, SyncErrorQueue, SyncQueueItem } from './kg-resilience.js';
      ...
      this.retryIntervalId = setInterval(async () => {
        if (this.syncErrorQueue.getQueueSize() > 0) {
          ...
          if (this.isProcessingRetryQueue) {
            console.log('[ConsciousMemory] Skipping retry queue processing as a previous run is still in progress');
            return;
          }
      ...
      try {        // Use a generic search to get all memories
        ...
        const filtered = combinedResults
          .filter(result => {
            ...
            // All memories are explicit in MCP context - apply filters directly
  ...
  async getAllTags(): Promise<string[]> {
    ...
    try {
      ...
      allMemories.forEach(memory => {
        ...
        // All memories are explicit in MCP context - no need to filter by memoryType
  ...
  async getStats(): Promise<ConsciousMemoryStats> {
    ...
    try {
      ...
      allMemories.forEach(memory => {
        ...
        totalImportance += (metadata.importance || 5);
      });

      return {
        totalConsciousMemories: allMemories.length,
        tagCount: tags.size,
